describe Money do
  ...
  describe "#/" do
    let(:money) { Money.new('12.34', 'EUR') }
    let(:result) { money / Money.new('2', 'EUR') }

    it "return a new money" do
      expect(result).to be_a(Money)
      expect(result).not_to be(money)
    end

    it "divide value of one money by the second one" do
      expect(result.value).to eq(6.17)
    end
  ...
  describe ".sum" do
    ...
    it "returns array with one element for one element array" do
      expect(Money.sum([little_euro])).to eq([little_euro])
    end

    it "returns two elements array when two different currencies are passed" do
      expect(Money.sum([little_euro, little_pln])).to eq([little_euro, little_pln])
    end

    it "sums all moneys matching currencies" do
  ...
  describe "#==" do
    let(:eur) { Money.new('10', 'EUR') }
    let(:rub) { Money.new('1234', 'RUB') }

    it "return false for EUR and RUB monies" do
      expect(eur == rub).to be_falsey
    end
  end

  describe "#the_same_currency" do
    let(:eur) { Money.new("123.321", "EUR") }

    it "returns true for two monies in EUR" do
      expect(Money.new('23', 'EUR').the_same_currency eur).to be_truthy
    end

    it "returns false for two different monies" do
      expect(Money.new('123', 'RUB').the_same_currency eur).to be_falsey
    end

  end

  describe "#different_currency" do
    let(:eur) { Money.new("123.321", "EUR") }

    it "returns false for the same currencies" do
      expect(eur.different_currency Money.new('12334.32', 'EUR')).to be_falsey
    end

    it "returns true for the different currencies" do
      expect(eur.different_currency Money.new('12.21', 'USD')).to be_truthy
    end
  end


  describe ".convert" do
    context "convert two elements array into instance of Money class" do
      let(:arry) { ['12.34', 'USD'] }
      let(:klass) { Money.convert(arry) }
      it "returns Money object which is filled with value and currency" do
        expect(klass).to be_a(Money)
        expect(klass.value).to eq(12.34)
        expect(klass.currency).to eq('USD')
      end
    end

    context "covert is not working with array with elements more than 2" do
      it "raises error when array is passed with three or more values" do
        expect {
          Money.convert(['12', 'USD', :foo])
        }.to raise_error(ArgumentError)
